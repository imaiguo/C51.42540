...
unsigned char  code Rx_Addr0[RX_ADDR_WITDH]={0xc0,0x43,0x10,0x10,0x01}; 
...
void NRFSetRXMode()													   //主要接收模式
{
	CE=0;  	
  	...
  	NRFWrite_Buf(WRITE_REG + RX_ADDR_P0 , Tx_Addr0 , TX_ADDR_WITDH);  // 接收设备接收通道0 和 发送设备的发送地址相同	     
  	NRFWriteReg(WRITE_REG + RX_PW_P0 , TX_DATA_WITDH);     // 接收通道0选择和发送通道相同有效数据宽度
