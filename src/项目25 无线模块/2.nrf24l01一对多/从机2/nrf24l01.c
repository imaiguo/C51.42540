...
void NRFSetRXMode()													   //主要接收模式
{
	CE=0;  	
  	...
  	NRFWrite_Buf(WRITE_REG + RX_ADDR_P2 , Tx_Addr2 , TX_ADDR_WITDH);  // 接收设备接收通道0 和 发送设备的发送地址相同	     
  	NRFWriteReg(WRITE_REG + RX_PW_P2 , TX_DATA_WITDH);     // 接收通道0选择和发送通道相同有效数据宽度
	NRFWriteReg(WRITE_REG + CONFIG, 0x0f);                 // CRC使能，16位CRC校验，上电  接收模式
  	NRFWriteReg(WRITE_REG + EN_RXADDR, 0x04);              // 使能接收通道0   
  	NRFWriteReg(WRITE_REG + EN_AA, 0x04);                  // 使能接收通道0自动应答
...
{
    CE=0; 
   	NRFWrite_Buf(WRITE_REG + TX_ADDR , Rx_Addr2, TX_ADDR_WITDH);    //写寄存器指令+接收地址使能指令，接收地址，地址宽度
	NRFWrite_Buf(WRITE_REG + RX_ADDR_P2 , Rx_Addr2 , TX_ADDR_WITDH); //为了应答接收设备，接收通道0地址和发送地址相同
